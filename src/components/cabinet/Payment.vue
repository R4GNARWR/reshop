<template>
    <div>
        <div class="cabinet__body cabinet__body--lg">
            <div class="cabinet__payment">
                <h2 class="cabinet__body-title">Платежные данные</h2>
                <h3 class="cabinet__payment-title">Для оплаты на сайте</h3>
                <div class="cabinet__payment-cards">
                    <a href="#" class="payment__card" v-if="cards.length">
                        <div class="payment__card-top">Основная карта</div>
                        <div class="payment__card-bottom">
                            <div class="payment__card-number">
                                **** **** **** **** 4665
                            </div>
                            <div class="payment__card-info">
                                <div class="payment__card-date">12/23</div>
                                <div class="payment__card-type">VISA</div>
                            </div>
                        </div>
                    </a>
                    <a href="#" class="payment__addcard">
                        <img
                        src="@/assets/images/add.svg"
                        alt=""
                        class="payment__addcard-img"
                        />
                        <p class="payment__addcard-text">Пока не доступно</p>
                    </a>
                </div>
                <div class="award">
                    <h3 class="cabinet__payment-title">
                        Для получения вознаграждения
                    </h3>
                    <p class="award__text">
                        Данные реквизиты вы можете посмотреть в мобильном приложении
                        своего банка
                    </p>
                    
                    <form class="award__form">
                        <div>
                            <label>Получатель</label>
                            <input
                            class="award__form-input"
                            type="text"
                            placeholder="Получатель"
                            v-model="account['receiver']"
                            />
                        </div>
                        <div>
                            <label>БИК банка получателя</label>
                            <input
                            class="award__form-input"
                            type="text"
                            placeholder="БИК банка получателя"
                            v-model="account['bik']"
                            />
                        </div>
                        
                        <div>
                            <label>Счет получателя</label>
                            <input
                            class="award__form-input"
                            type="text"
                            placeholder="Счет получателя"
                            v-model="account['number']"
                            />
                        </div>
                        
                        <!--                    <input-->
                        <!--                        class="award__form-input"-->
                        <!--                        type="text"-->
                        <!--                        placeholder="Корреспондентский счет"-->
                        <!--                    />-->
                        <div>
                            <label>ИНН получателя</label>
                            <input
                            class="award__form-input"
                            type="text"
                            placeholder="ИНН получателя"
                            v-model="account['inn']"
                            />
                        </div>
                        
                    </form>
                    <div class="cabinet__commision">
                        <svg
                        width="14"
                        height="16"
                        viewBox="0 0 14 16"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        >
                        <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M14 4.5V14C14 14.5304 13.7893 15.0391 13.4142 15.4142C13.0391 15.7893 12.5304 16 12 16V15C12.2652 15 12.5196 14.8946 12.7071 14.7071C12.8946 14.5196 13 14.2652 13 14V4.5H11C10.6022 4.5 10.2206 4.34196 9.93934 4.06066C9.65804 3.77936 9.5 3.39782 9.5 3V1H4C3.73478 1 3.48043 1.10536 3.29289 1.29289C3.10536 1.48043 3 1.73478 3 2V11H2V2C2 1.46957 2.21071 0.960859 2.58579 0.585786C2.96086 0.210714 3.46957 0 4 0L9.5 0L14 4.5ZM6.161 13.666V14.188C6.161 14.444 6.122 14.658 6.044 14.829C5.97762 14.9872 5.86549 15.122 5.722 15.216C5.58064 15.3013 5.41808 15.345 5.253 15.342C5.08729 15.345 4.92406 15.3014 4.782 15.216C4.63969 15.1216 4.52837 14.9873 4.462 14.83C4.37788 14.6267 4.338 14.4079 4.345 14.188V13.666C4.345 13.409 4.385 13.195 4.462 13.025C4.52821 12.8673 4.63953 12.7327 4.782 12.638C4.92342 12.5508 5.08689 12.506 5.253 12.509C5.429 12.509 5.585 12.552 5.722 12.639C5.86534 12.7328 5.97745 12.8672 6.044 13.025C6.122 13.195 6.161 13.409 6.161 13.666ZM6.964 14.185V13.672C6.964 13.295 6.896 12.972 6.759 12.7C6.63059 12.4355 6.42529 12.2159 6.17 12.07C5.916 11.923 5.61 11.85 5.253 11.85C4.898 11.85 4.591 11.923 4.333 12.07C4.07741 12.2142 3.87191 12.4329 3.744 12.697C3.608 12.968 3.539 13.293 3.539 13.672V14.185C3.539 14.56 3.608 14.885 3.744 15.158C3.881 15.429 4.077 15.638 4.334 15.785C4.591 15.929 4.898 16.001 5.254 16.001C5.611 16.001 5.916 15.929 6.17 15.785C6.426 15.638 6.622 15.429 6.76 15.158C6.896 14.884 6.964 14.56 6.964 14.185ZM0 11.926V15.926H1.459C1.861 15.926 2.194 15.846 2.458 15.688C2.72516 15.5282 2.93377 15.2866 3.053 14.999C3.183 14.699 3.249 14.337 3.249 13.915C3.249 13.495 3.184 13.137 3.053 12.84C2.93492 12.5556 2.72794 12.317 2.463 12.16C2.2 12.004 1.865 11.926 1.459 11.926H0ZM0.791 12.571H1.354C1.602 12.571 1.804 12.621 1.963 12.723C2.12804 12.8309 2.25256 12.9906 2.317 13.177C2.396 13.378 2.435 13.629 2.435 13.93C2.4381 14.1294 2.41524 14.3285 2.367 14.522C2.33135 14.6746 2.26463 14.8183 2.171 14.944C2.08399 15.0561 1.96872 15.1431 1.837 15.196C1.68266 15.2537 1.51875 15.2816 1.354 15.278H0.79V12.57L0.791 12.571ZM8.213 13.054C8.13956 13.2567 8.10463 13.4714 8.11 13.687V14.182C8.11 14.428 8.144 14.637 8.213 14.809C8.27044 14.967 8.37432 15.104 8.511 15.202C8.65366 15.2925 8.82016 15.3381 8.989 15.333C9.12778 15.3362 9.26529 15.306 9.39 15.245C9.50156 15.1883 9.59584 15.1026 9.663 14.997C9.73085 14.8869 9.77099 14.762 9.78 14.633H10.545V14.709C10.5355 14.9506 10.4571 15.1845 10.319 15.383C10.182 15.577 9.999 15.728 9.769 15.837C9.52258 15.9498 9.25397 16.0059 8.983 16.001C8.623 16.001 8.319 15.929 8.069 15.785C7.81941 15.6388 7.62024 15.4201 7.498 15.158C7.368 14.886 7.304 14.561 7.304 14.182V13.684C7.304 13.305 7.37 12.979 7.501 12.706C7.631 12.432 7.822 12.221 8.072 12.073C8.324 11.924 8.628 11.85 8.983 11.85C9.202 11.85 9.404 11.882 9.59 11.947C9.777 12.009 9.94 12.1 10.079 12.219C10.2194 12.3385 10.3334 12.4859 10.4136 12.6519C10.4939 12.8179 10.5386 12.9987 10.545 13.183V13.256H9.78C9.77068 13.1216 9.72955 12.9914 9.66 12.876C9.59463 12.7656 9.5002 12.6753 9.387 12.615C9.26497 12.5488 9.12783 12.5153 8.989 12.518C8.82019 12.5136 8.65419 12.5618 8.514 12.656C8.37687 12.7563 8.27221 12.8947 8.213 13.054Z"
                        fill="black"
                        />
                    </svg>
                    <router-link to="/">Договор комиссии</router-link>
                    
                    <a class="content__save btn" style="margin-top: 20px; max-width: 300px"
                    @click="saveIt()">Сохранить</a>
                    
                </div>
            </div>
        </div>
    </div>
    <!-- Мобильный Личный Кабинет -> Платежные данные-->
    <div class="mob-payment mob-cabinet__body">
        <h1 class="mob-cabinet__title">Личный кабинет</h1>
        <div class="mob-cabinet__tabs">
            <a href="#" class="mob-cabinet__tab active" @click.prevent="$emit('close')">Платежные данные</a>
        </div>
        <div class="cabinet__payment">
            <h2 class="cabinet__body-title">Платежные данные</h2>
            <h3 class="cabinet__payment-title">Для оплаты на сайте</h3>
            <div class="cabinet__payment-cards">
                <a href="#" class="payment__card">
                    <div class="payment__card-top">Основная карта</div>
                    <div class="payment__card-bottom">
                        <div class="payment__card-number">
                            **** **** **** **** 4665
                        </div>
                        <div class="payment__card-info">
                            <div class="payment__card-date">12/23</div>
                            <div class="payment__card-type">VISA</div>
                        </div>
                    </div>
                </a>
                <a href="#" class="payment__addcard">
                    <img
                    src="@/assets/images/add.svg"
                    alt=""
                    class="payment__addcard-img"
                    />
                    <p class="payment__addcard-text">Добавить карту</p>
                </a>
            </div>
            <div class="award">
                <h3 class="cabinet__payment-title">
                    Для получения вознаграждения
                </h3>
                <p class="award__text">
                    Данные реквизиты вы можете посмотреть в мобильном приложении
                    своего банка
                </p>
                
                <form class="award__form">
                    <label>БИК банка получателя</label>
                    <input
                    class="award__form-input"
                    type="text"
                    placeholder="БИК банка получателя"
                    name="bik"
                    />
                    <label>Банк получателя</label>
                    <input
                    class="award__form-input"
                    type="text"
                    placeholder="Банк получателя"
                    />
                    <label >Счет получателя</label>
                    <input
                    class="award__form-input"
                    type="text"
                    placeholder="Счет получателя"
                    />
                    <!-- <label>Корреспондентский счет</label>
                        <input
                        class="award__form-input"
                        type="text"
                        placeholder="Корреспондентский счет"
                        /> -->
                        <label>ИНН получателя</label>
                        <input
                        class="award__form-input"
                        type="text"
                        placeholder="ИНН получателя"
                        />
                    </form>
                    <div class="cabinet__commision">
                        <svg
                        width="14"
                        height="16"
                        viewBox="0 0 14 16"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        >
                        <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M14 4.5V14C14 14.5304 13.7893 15.0391 13.4142 15.4142C13.0391 15.7893 12.5304 16 12 16V15C12.2652 15 12.5196 14.8946 12.7071 14.7071C12.8946 14.5196 13 14.2652 13 14V4.5H11C10.6022 4.5 10.2206 4.34196 9.93934 4.06066C9.65804 3.77936 9.5 3.39782 9.5 3V1H4C3.73478 1 3.48043 1.10536 3.29289 1.29289C3.10536 1.48043 3 1.73478 3 2V11H2V2C2 1.46957 2.21071 0.960859 2.58579 0.585786C2.96086 0.210714 3.46957 0 4 0L9.5 0L14 4.5ZM6.161 13.666V14.188C6.161 14.444 6.122 14.658 6.044 14.829C5.97762 14.9872 5.86549 15.122 5.722 15.216C5.58064 15.3013 5.41808 15.345 5.253 15.342C5.08729 15.345 4.92406 15.3014 4.782 15.216C4.63969 15.1216 4.52837 14.9873 4.462 14.83C4.37788 14.6267 4.338 14.4079 4.345 14.188V13.666C4.345 13.409 4.385 13.195 4.462 13.025C4.52821 12.8673 4.63953 12.7327 4.782 12.638C4.92342 12.5508 5.08689 12.506 5.253 12.509C5.429 12.509 5.585 12.552 5.722 12.639C5.86534 12.7328 5.97745 12.8672 6.044 13.025C6.122 13.195 6.161 13.409 6.161 13.666ZM6.964 14.185V13.672C6.964 13.295 6.896 12.972 6.759 12.7C6.63059 12.4355 6.42529 12.2159 6.17 12.07C5.916 11.923 5.61 11.85 5.253 11.85C4.898 11.85 4.591 11.923 4.333 12.07C4.07741 12.2142 3.87191 12.4329 3.744 12.697C3.608 12.968 3.539 13.293 3.539 13.672V14.185C3.539 14.56 3.608 14.885 3.744 15.158C3.881 15.429 4.077 15.638 4.334 15.785C4.591 15.929 4.898 16.001 5.254 16.001C5.611 16.001 5.916 15.929 6.17 15.785C6.426 15.638 6.622 15.429 6.76 15.158C6.896 14.884 6.964 14.56 6.964 14.185ZM0 11.926V15.926H1.459C1.861 15.926 2.194 15.846 2.458 15.688C2.72516 15.5282 2.93377 15.2866 3.053 14.999C3.183 14.699 3.249 14.337 3.249 13.915C3.249 13.495 3.184 13.137 3.053 12.84C2.93492 12.5556 2.72794 12.317 2.463 12.16C2.2 12.004 1.865 11.926 1.459 11.926H0ZM0.791 12.571H1.354C1.602 12.571 1.804 12.621 1.963 12.723C2.12804 12.8309 2.25256 12.9906 2.317 13.177C2.396 13.378 2.435 13.629 2.435 13.93C2.4381 14.1294 2.41524 14.3285 2.367 14.522C2.33135 14.6746 2.26463 14.8183 2.171 14.944C2.08399 15.0561 1.96872 15.1431 1.837 15.196C1.68266 15.2537 1.51875 15.2816 1.354 15.278H0.79V12.57L0.791 12.571ZM8.213 13.054C8.13956 13.2567 8.10463 13.4714 8.11 13.687V14.182C8.11 14.428 8.144 14.637 8.213 14.809C8.27044 14.967 8.37432 15.104 8.511 15.202C8.65366 15.2925 8.82016 15.3381 8.989 15.333C9.12778 15.3362 9.26529 15.306 9.39 15.245C9.50156 15.1883 9.59584 15.1026 9.663 14.997C9.73085 14.8869 9.77099 14.762 9.78 14.633H10.545V14.709C10.5355 14.9506 10.4571 15.1845 10.319 15.383C10.182 15.577 9.999 15.728 9.769 15.837C9.52258 15.9498 9.25397 16.0059 8.983 16.001C8.623 16.001 8.319 15.929 8.069 15.785C7.81941 15.6388 7.62024 15.4201 7.498 15.158C7.368 14.886 7.304 14.561 7.304 14.182V13.684C7.304 13.305 7.37 12.979 7.501 12.706C7.631 12.432 7.822 12.221 8.072 12.073C8.324 11.924 8.628 11.85 8.983 11.85C9.202 11.85 9.404 11.882 9.59 11.947C9.777 12.009 9.94 12.1 10.079 12.219C10.2194 12.3385 10.3334 12.4859 10.4136 12.6519C10.4939 12.8179 10.5386 12.9987 10.545 13.183V13.256H9.78C9.77068 13.1216 9.72955 12.9914 9.66 12.876C9.59463 12.7656 9.5002 12.6753 9.387 12.615C9.26497 12.5488 9.12783 12.5153 8.989 12.518C8.82019 12.5136 8.65419 12.5618 8.514 12.656C8.37687 12.7563 8.27221 12.8947 8.213 13.054Z"
                        fill="black"
                        />
                    </svg>
                    <a href="#">Договор комиссии</a>
                </div>
            </div>
        </div>
    </div>
    
    <!--     todo: сохранить данные карты -->
    
    
    <!-- {{user_info}} -->
</div>
</template>

<script>
import api from "@/api";
import {useSessionStore} from "@/store/session";
export default {
    data(){
        return{
            cards:[], account:{}
        }
    },
    computed:{
        user_info(){
            return useSessionStore().user_info}
        },
        methods:{
            saveIt(){
                let userData = {
                    userId:useSessionStore().user_info.id,
                    type:"accounts",
                    json:JSON.stringify(this.account)
                }
                api.putRequest("user-data",userData)
                //cards
                //account
            }
        },
        created() {
            if(useSessionStore().user_info && useSessionStore().user_info["accounts"])
            this.account = useSessionStore().user_info["accounts"]
        }
        
        
    }
</script>

<style>

</style>
